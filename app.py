                ...
                try:
                    ...
                    response = client.chat.completions.create(
                        ...
                        messages=[{"role": "user", "content": [
                            {"type": "text", "text": "Extract the recipe EXACTLY as JSON only. Preserve all quantities, units, and original text COMPLETELY - do not truncate ANY ingredient lines, including long serving suggestions like 'FOR SERVING: Chopped onion, cilantro, etc.'. Keep steps exactly as shown. Format: {\"title\": \"...\", \"ingredients\": [\"...\"], \"steps\": [\"...\"]}"},
...
Preserve exact ingredient lines and step phrasing COMPLETELY - do not truncate ANY ingredient lines, including complete serving suggestions like 'FOR SERVING: Chopped onion, cilantro, etc.'
...
{page_text}"""}],
            ...
            if text_input.strip():
                try:
                    response = client.chat.completions.create(
                        model="llama-3.3-70b-versatile",
                        messages=[{"role": "user", "content": f"Extract the recipe EXACTLY as JSON only. Preserve everything COMPLETELY - do not truncate ANY ingredient lines, including complete serving suggestions. Format: {{\"title\": \"...\", \"ingredients\": [...], \"steps\": [...]}}\n\nText:\n{text_input}"}],
                        max_tokens=1500
                    )
                    recipe = parse_recipe_json(response.choices[0].message.content)
                    if recipe and recipe.get('title') and recipe.get('ingredients'):
